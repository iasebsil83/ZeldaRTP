...
#
core.clear()
core.d2D(1,5,"   ^V  V^^V^V ^V V^")
core.d2D(1,6," 0  /  /  / /  / /")
core.d2D(1,7,"({~~H~~H~~H~H~~H~H~~~")
if story < 6:
    core.d2D(1,1,"[ Je savais que tu  ]")
    core.d2D(1,2,"[arriverais un jour ]")
    core.d2D(1,3,"=======[enter]=======")
    core.show()
    key = voidKey
    while key != 13:
        key = core.getKey()
    core.d2D(2,1,"pour nous sauver de")
    core.d2D(2,2,"Barinade, l'anémone")
    core.show()
    key = voidKey
    while key != 13:
        key = core.getKey()
    core.d2D(2,1,"infernale. Tiends, ")
    core.d2D(2,2,"prends ca guerrier!")
    core.show()
    key = voidKey
    while key != 13:
        key = core.getKey()
    core.d2D(2,1,"  Tu as obtenu un  ")
    core.d2D(2,2,"     Grappin !!!   ")
    core.show()
    key = voidKey
    while key != 13:
        key = core.getKey()
    ...
    core.d2D(1,3,"                     ")
    story = 6
x,y,hookOk,hookX,hookY = 2,7,0,0,0
if Wx != 0:
	x = Wx
	y = Wy
right = True
