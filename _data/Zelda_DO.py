...
#
core.clear()
x,y,Bcnt,Blife,Bpos,bombCnt,bombPos = 3,6,0,7,20,0,0
if Wx != 0:
    x = Wx
    y = Wy
right = True
core.d2D(1,1,"Vie:")
core.d2D(1,7,"_____________________")
key = 1
while key != 105:
    if key != voidKey or bombCnt == 1:
    ...
    if bombCnt == 1:
        ...
        if bCooX+1 != 22:
            core.d2D(bCooX+1,6,"X")
        if bCooX < Bpos+3 and bCooX > Bpos-3:
            Blife -= 1
    ...
    if Blife != 7:
        Bcnt += 1
    if Bcnt == 5:
        if x == Bpos or x == Bpos-1 or x == Bpos+1:
            life -= 1
        if x < Bpos:
            Bpos -= 1
        if x > Bpos:
