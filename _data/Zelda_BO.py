...
#
core.clear()
x,Spos,Sright,Scnt,refresh,bombCnt,bCooX = 20,6,0,0,0,0,0
if Wx != 0:
    x = Wx
    y = Wy
right = True
...
key = 1
while key != 105:
    if key != voidKey or Scnt == 2 or refresh == 1:
        refresh = 0
    ...
    if key == 122:
        if x == 6:
            ...
            x = 6
    if key == sword and x != 1 and x != 21 and story != 0:
    ...
    if key == 13 and x == 2 and story == 0:
        story = 1
        refresh = 1
        core.d2D(1,1,"---------------------")
        core.d2D(1,2,"|Tu as obtenu l'epee|")
        core.d2D(1,3,"|Kokiri !   ...     |")
        core.d2D(1,4,"-------[enter]-------")
        core.show()
        key = voidKey
        while key != 13:
            key = core.getKey()
        key = voidKey
        core.d2D(2,2,"Oh! Un chemin s`est")
        core.d2D(2,3,"ouvert !          ")
        core.show()
        while key != 13:
            key = core.getKey()
